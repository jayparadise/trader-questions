    ...
    if (imageMatch) {
      elements.push(
        <div key={i} style={{ margin: '12px 0' }}>
          <img src={imageMatch[1]} alt="Reference screenshot" style={{ maxWidth: '100%', borderRadius: 8, border: '1px solid var(--border)', display: 'block' }} onError={e => { e.currentTarget.style.display='none' }} />
          <div style={{ fontSize: 10, color: 'var(--text-3)', marginTop: 3, fontFamily: 'var(--font-mono)' }}>Reference screenshot</div>
        </div>
      )
      i++; continue
    }
    ...
    if (line.startsWith('### ')) {
      ...
      elements.push(
        <div key={i} style={{ display: 'flex', gap: 10, marginBottom: 6, alignItems: 'flex-start' }}>
          <span style={{ minWidth: 20, height: 20, borderRadius: '50%', background: 'var(--teal)', color: '#fff', fontSize: 10, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 2, flexShrink: 0 }}>{num}</span>
          <span style={{ lineHeight: 1.65 }}>{renderInline(line.replace(/^\d+\. /,''))}</span>
        </div>
      )
    ...
    <div style={{ display:'flex', height:'100vh', overflow:'hidden' }}>
      ...
      <div style={{ width:248, flexShrink:0, background:'var(--bg-sidebar)', display:'flex', flexDirection:'column', overflowY:'auto' }}>
        ...
        <div style={{ padding:'20px 16px 16px', borderBottom:'1px solid rgba(255,255,255,0.07)' }}>
          ...
          <div style={{ fontSize:16, fontWeight:700, color:'#fff', lineHeight:1.3 }}>
            ...
            <span style={{ color:'#5DDDC8', fontWeight:400, fontSize:13 }}>Assistant</span>
          </div>
        </div>
        ...
        <div style={{ padding:'10px 12px 0' }}>
          ...
          {kbOpen && (
            <div style={{ marginTop:6, display:'flex', flexDirection:'column', gap:2 }}>
              {DOCS.map((doc,i) => (
                <button key={i} onClick={()=>send(doc.query)} style={{ background:'none', border:'none', padding:'4px 6px', fontSize:11, color:'rgba(255,255,255,0.5)', textAlign:'left', cursor:'pointer', borderRadius:5, transition:'all 0.12s', fontFamily:'var(--font-body)' }}
        ...
        <div style={{ padding:'0 12px', flex:1, display:'flex', flexDirection:'column', gap:10, paddingBottom:16 }}>
          {QUESTION_SECTIONS.map((section, si) => (
            <div key={si}>
              ...
              {openSections[si] && (
                <div style={{ marginTop:4, display:'flex', flexDirection:'column', gap:1, paddingLeft:4 }}>
                  {section.questions.map((q,i) => (
                    <button key={i} onClick={()=>send(q)} style={{ background:'none', border:'none', padding:'5px 8px', fontSize:12, color:'rgba(255,255,255,0.5)', textAlign:'left', cursor:'pointer', borderRadius:6, transition:'all 0.12s', lineHeight:1.4, fontFamily:'var(--font-body)' }}
                      onMouseEnter={e=>{e.currentTarget.style.background='rgba(93,221,200,0.1)';e.currentTarget.style.color='#5DDDC8'}}
                      onMouseLeave={e=>{e.currentTarget.style.background='none';e.currentTarget.style.color='rgba(255,255,255,0.5)'}}>
                      {q}
                    </button>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', background:'#fff' }}>
        ...
        <div style={{ height:48, borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 24px', flexShrink:0 }}>
          <span style={{ fontSize:13, color:'var(--text-3)' }}>Ask anything about trading procedures</span>
